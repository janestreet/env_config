...
module Load_from_disk = struct
  ...
  let prepare { save_default_configuration_to_disk; environment_override } ~temp_dir =
    let%bind () =
      if save_default_configuration_to_disk then begin
        let%bind path = default_config_path () in
        Writer.save_sexp
          path
          (Sexp.of_string "Read_from_default_file")
    ...
    | `File       ->
      ...
      let%bind () =
        Writer.save_sexp
          temp_file
          (Sexp.of_string "Read_from_environment_specified_file")
      in
...
let test ?blocking ?async ?(load_from_disk : Load_from_disk.t option) () =
  with_temp_dir (fun temp_dir ->
    let env var value =
      Option.map value ~f:(fun value -> var, value)
      |> Option.to_list
    in
    ...
    let extend_env =
      ...
      ; env "ASYNC" async
      ; from_disk_environment
      ] |> List.concat
    in
    let%bind () =
      run ~extend_env "../example/example.exe"
...
let%expect_test "config is on disk, no environment" =
  let load_from_disk =
    { Load_from_disk.
      save_default_configuration_to_disk = true
...
let%expect_test "config on not on disk, filename in environment" =
  let load_from_disk =
    { Load_from_disk.
      save_default_configuration_to_disk = false
    ; environment_override = `File
    }
  in
  let%bind () = test ~load_from_disk () in
